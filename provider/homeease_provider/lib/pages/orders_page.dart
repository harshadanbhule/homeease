      ...
      body: FutureBuilder<List<OrderDetails>>(
        ...
        builder: (context, snapshot) {
          ...
          if (filteredOrders.isEmpty) {
            return Center(
              child: Text(
                "No orders found for ${userDetailController.serviceName.value} service.",
              ),
            );
          }
          ...
          final acceptedOrders =
              filteredOrders.where((order) => order.order.isAccepted).toList();
          final pendingOrders =
              filteredOrders.where((order) => !order.order.isAccepted).toList();
      ...
      child: InkWell(
        onTap: () {
          Navigator.push(
            context,
            MaterialPageRoute(
              builder: (context) => OrderDetailsPage(order: order, user: user),
            ),
          );
        },
        child: Padding(
